...
<script>
  ...
  // Same host that served this page, whatever address that is. Hardcoding
  // 192.168.4.1 only worked while the board was its own access point.
  const status = document.getElementById("status");
  var ws = new WebSocket(`ws://${location.host}/ws`);
  ws.onopen = () => { status.textContent = "connected"; console.log("WebSocket connected"); };
  ws.onmessage = e => console.log("WebSocket message: " + e.data);
  ws.onclose = () => { status.textContent = "disconnected"; console.log("WebSocket closed"); };
  ws.onerror = e => { status.textContent = "error"; console.log("WebSocket error: " + e); };
  ...
  function sendMessage(messageType) {
    // Clicking a button before the socket opens throws InvalidStateError.
